...
# Inside your Osintgram class
class Osintgram:
    def __init__(self, id, file, json, command, output, cookies):
        # Your existing code
    ...
    def check_following(self):
        try:
            endpoint = # Replace with the correct Instagram API endpoint for checking following status
            response = self.api._call_api(endpoint)
            return response['user_detail']['user']['friendship_status']['following']
        except instagram_private_api.errors.HTTPError as e:
            if e.code == 404:
                print("User not found.")
            else:
                print(f"HTTP Error: {e.code} - {e.msg}")
            return None  # or handle the error in an appropriate way based on your application's logic
